...
ALTER TABLE products
ADD COLUMN IF NOT EXISTS salla_store_id UUID REFERENCES stores (id);

ALTER TABLE products ADD COLUMN IF NOT EXISTS zid_product_id TEXT;

ALTER TABLE products
ADD COLUMN IF NOT EXISTS zid_store_id UUID REFERENCES stores (id);

-- Backfill from existing store_product_id + store_id based on store platform
UPDATE products p
SET
    salla_product_id = p.store_product_id,
...
UPDATE products p
SET
    zid_product_id = p.store_product_id,
